...
public class SellBarrel extends Barrel {
	...
	public SellBarrel(String label, String coords, String bid_price, double compressed_bid_price, int currency_type) {
		super(label, coords);
		this.bid_price = bid_price;
		this.bid_str = bid_price.replace("sell for", "").trim();
		this.compressed_bid_price = compressed_bid_price;
		this.currency_type = currency_type;
		this.barrel_type = BarrelTypes.SELL;
	}
	...
	public void convertSolutionToCompressed() {
		...
		double expected_compressed = (other_items > 0) ? -1.0*other_items*compressed_bid_price : 0;
	...
	public void generateGuiText() {
		...
		MutableText buy_for = Text.literal("Sell For: ");
		buy_for = buy_for.withColor(red_color);
		buy_for.append(Text.literal("%s".formatted(bid_str)).withColor(light_blue_color));
	...
	public boolean validateTransaction() {
		...
		double expected_compressed = (other_items > 0 && actual_compressed <= 0) ? -1.0*other_items*compressed_bid_price : 0;
	     ...
	     *  Put money back
	     *  if currency_delta%price == 0: (OR) put mats.
	     *  
	     * + mats, 0 currency
		...
		if(other_items < 0) build_mistrade_text.append("\nBought mats instead of sold mats!");
		build_mistrade_text.append("\n§7--------------------");
